...
{
    self = [self init];
    _model = cv::createEigenFaceRecognizer(9,1600.0);
...
- (NSDictionary *)recognizeFace:(cv::Rect)face inImage:(cv::Mat&)image
{
   ...
   if (predictedLabel != -1) {
        ...
        NSEntityDescription *entity = [NSEntityDescription entityForName:@"Faces" inManagedObjectContext:context1];
        [fetchRequest setEntity:entity];
        fetchRequest.predicate = [NSPredicate predicateWithFormat:@"label== %@",predictedLabel];
        NSError *error = nil;
        NSArray *array = [context1 executeFetchRequest:fetchRequest error:&error];
        if(array ==nil)
        {
       ...
       /* const char* selectSQL = "SELECT name FROM person WHERE personid =?";//"SELECT name FROM people WHERE id = ?";
        sqlite3_stmt *statement;
        //NSLog(@"este es el predictedLabel %d", predictedLabel);
